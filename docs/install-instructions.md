...
The following modules are required for `PSDocs.Azure` to work:

- PSDocs

The required version of each module will automatically be installed along-side `PSDocs.Azure` when using `Install-Module` or `Update-Module` cmdlets.
...
Save-Module -Name 'PSDocs', 'PSDocs.Azure' -Path '.\modules';
```
