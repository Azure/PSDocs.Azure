...
namespace PSDocs.Azure.Pipeline.Output
{
    ...
    {
        private const string Source = "PSDocs";
        ...
        internal PSPipelineWriter(PSDocumentOption option)
            : base(null, option) { }
        ...
        {
            if (OnWriteVerbose == null || !ShouldWriteVerbose() || string.IsNullOrEmpty(message))
                return;

            OnWriteVerbose(string.Concat(VerbosePrefix, message));
        }
